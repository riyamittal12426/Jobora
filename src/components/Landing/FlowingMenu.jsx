...
function MenuItem({ link, text, image, speed, textColor, marqueeBgColor, marqueeTextColor, borderColor, isFirst }) {
  ...
  return (
    <div
      className="flex-1 relative overflow-hidden text-center h-full flex flex-col justify-center"
      ref={itemRef}
      style={{ borderTop: isFirst ? 'none' : `1px solid ${borderColor}` }}
    >
      <a
        className="flex items-center justify-center h-full relative cursor-pointer uppercase no-underline font-semibold text-[2vh] tracking-wider"
        href={link}
        ...
        <div className="h-full w-fit flex" ref={marqueeInnerRef}>
          {[...Array(repetitions)].map((_, idx) => (
            <div className="marquee-part flex items-center flex-shrink-0 h-full" key={idx} style={{ color: marqueeTextColor }}>
              <span className="whitespace-nowrap uppercase font-normal text-[2vh] leading-[1] px-[1vw] tracking-widest">{text}</span>
              <div
                className="w-[100px] h-[5vh] my-[2em] mx-[1vw] py-[1em] rounded-[50px] bg-cover bg-center"
